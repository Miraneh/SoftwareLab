...
تست‌هایی که با خطا مواجه شدند یکی setWidthOrLength است که در آن چون یک مستطیل به یک مربع اشاره می‌کند و سپس با استفاده از قابلیت‌های مستطیل طول شکل را عوض می‌کنیم دیگر این شکل مربع نخواهد بود. یکی دیگر از تست‌ها testInheritance در تست‌های مربع است. در این تست اصل Liskov که می‌گوید باید بتوانیم آبجکت‌های پدر را با پسر بدون خطا جابجا کنیم، زیر پا می‌رود. پس این کد نیاز به تغییراتی دارد. کد را به صورتی تغییر می‌دهیم که هر دو شکل از یک اینترفیس به نام Shape استفاده کنند تا این خطاهای مربوط به ارث‌بری پیش نیایند. تست‌ّهایی که برای ارزیابی این روش تولید کردیم به شکل زیر هستند:
